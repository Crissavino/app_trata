...
<header>
	...
	<ul class="nav nav-tabs">
        ...
        @endif
        @if ($idFormB)
            <li class="nav-item"> <a class="nav-link active" href="/formularios/edicion/B/{{ $idCarpeta }}/{{ $idFormB }}">Eje B: Caracterización de la víctima</a> </li>
        @else
            <li class="nav-item"> <a class="nav-link active" href="/formularios/B/{{ $idCarpeta }}">Eje B: Caracterización de la víctima</a> </li>
        @endif
        @if ($idFormC)
...
<body>
	<section class="container">
        ...
        <form class="formulario" id="formularioB" action="{{$Bformulario->id}}" method="post">
            ...
            @else
                ...
                <!-- VIGESIMATERCER PREGUNTA -->
                    <div class="form-group {{ $errors->has('vicindirecta_id') ? 'has-error' : ''}}">
                        <label for="">B 23. ¿Tiene asociadas víctimas indirectas?:</label>
                        @if (auth()->user()->isAdmin !== 2)
                            <select class="selectVictimasIndirectas form-control" name="vicindirecta_id" id="vicindirecta_id">
                                <option value="" disabled selected>Seleccione</option>
                                @foreach ($datosVictimaIndirecta as $vicIndirecta)
                                    @php
                                        $selected = ($vicIndirecta->id == $Bformulario->vicindirecta_id) ? 'selected' : '';
                                    @endphp
                                    <option value="{{$vicIndirecta->id}}" {{ $selected }}>{{$vicIndirecta->nombre}}</option>
                                @endforeach
                            </select>
                        @else
                            <select disabled class="form-control" name="vicindirecta_id" id="vicindirecta_id">
                                @foreach ($datosVictimaIndirecta as $vicIndirecta)
                                    @php
                                        $selected = ($vicIndirecta->id == $Bformulario->vicindirecta_id) ? 'selected' : '';
                                    @endphp
                        ...
                        @endif
                        {!! $errors->first('vicindirecta_id', '<p class="help-block" style="color:red";>:message</p>') !!}
                    </div>
                <!-- FIN VIGESIMATERCER PREGUNTA -->

                {{-- PREGUNTAS B24 Y OTRAS --}}
                    <div class="tieneVictimasIndirectas">
                        <p>Todas las preguntas b24 y otras</p>
                    </div>
                {{-- FIN PREGUNTAS B24 Y OTRAS --}}
            @endif
    ...
    <script>
        ...
        //                         }
        //                         else{
        //                             alert("no es posible registrar correctamente el lugar de nacimiento. Vuelva a hacer click en el botón Actualizar");
        //                             mal_registro=true;
        //                         }                                    
        ...
        //         }
        //         if(mal_registro) return false;
        //         window.setTimeout(function(){formulario.submit()}, 3000)
        //     };//)
