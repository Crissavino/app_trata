...
<header>
    ...
    <ul class="nav nav-tabs">
        ...
        <li class="nav-item">
            <a class="nav-link " href="/formularios/edicion/D">Eje D: Datos de delito</a> </li> --}} {{--
        <li class="nav-item">
            <a class="nav-link " href="/formularios/edicion/E">Eje E: Datos del imputado</a> </li> --}} {{--
        <li class="nav-item">
            <a class="nav-link " href="/formularios/edicion/F">Eje F: Atención del caso</a> </li> --}} {{--
        <li class="nav-item">
            <a class="nav-link " href="/formularios/edicion/G">Eje G: Documentación</a> </li> --}}
    </ul>
</header>
    ...
    <section class="container">
        <form class="" id="formularioA" action="{{$aFormulario->id}}" method="post">
            ...
            <div class="form-group" {{ $errors->has('datos_numero_carpeta') ? 'has-error' : ''}}>
                <label for="datos_numero_carpeta">A 2. Número de carpeta:</label> @if (auth()->user()->isAdmin !== 2 && $usuarioCarpeta
                == auth()->user()->id)
            ...
            <div class="form-group" {{ $errors->has('datos_fecha_ingreso') ? 'has-error' : ''}}>
                <label for="datos_fecha_ingreso">A 3. Fecha de Ingreso:</label> @if (auth()->user()->isAdmin !== 2 && $usuarioCarpeta
                == auth()->user()->id)
            ...
            <div class="form-group" {{ $errors->has('modalidad_id') ? 'has-error' : ''}}>
                <label for="modalidad_id">A 4. Modalidad de Ingreso</label> @if (auth()->user()->isAdmin !== 2 && $usuarioCarpeta
                == auth()->user()->id)
...
@endphp
                ...
                <div id="presentacion_espontanea_id" class="presentacion_espontanea" style="display: none;" {{ $errors->has('presentacion_espontanea_id') ? 'has-error' : ''}}><br> @if (auth()->user()->isAdmin !== 2 && $usuarioCarpeta
                    == auth()->user()->id)
...
@endphp
                ...
                <div id="derivacion_otro_organismo_id" class="derivacion_otro_organismo" style="display: none;" {{ $errors->has('derivacion_otro_organismo_id') ? 'has-error' : ''}}><br> @if (auth()->user()->isAdmin !== 2 && $usuarioCarpeta
                    == auth()->user()->id)
...
@endphp
                ...
                <div id="derivacion_otro_organismo_cual" class="derivacion_otro_organismo_cual" style="display: none;" {{ $errors->has('derivacion_otro_organismo_cual') ? 'has-error' : ''}}>
                    <br><label for="">Cuál?</label> @if (auth()->user()->isAdmin !== 2 && $usuarioCarpeta == auth()->user()->id)
            ...
            <div class="form-group" {{ $errors->has('estadocaso_id') ? 'has-error' : ''}}>
                <label for="estadocaso_id">A 5. Estado Actual:</label> @if (auth()->user()->isAdmin !== 2 && $usuarioCarpeta
                == auth()->user()->id)
...
@endphp
            ...
            @endif {{-- FIN SEXTA PREGUNTA --}} {{-- INICIO SEPTIMA PREGUNTA --}}
            <div class="form-group" {{ $errors->has('caratulacionjudicial_id') ? 'has-error' : ''}}>
                <label for="caratulacionjudicial_id">A 7. Caratulación Judicial:</label> @if (auth()->user()->isAdmin !==
                2 && $usuarioCarpeta == auth()->user()->id)
...
@endphp
                                <option value="{{ $caratulacion->id }}" {{ $selected }}>{{ $caratulacion->nombre }}</option>
                            @endforeach
                        </select> @endif {!! $errors->first('caratulacionjudicial_id', '
                <p class="help-block" style="color:red" ;>:message</p>') !!}
            ...
            </script> --}} {{-- FIN SEPTIMA PREGUNTA --}} {{-- INICIO OCTAVA PREGUNTA --}}
            <div class="form-group" {{ $errors->has('datos_nro_causa') ? 'has-error' : ''}}>
                <label for="datos_nro_causa">A 8. N° Causa o Id Judicial:</label> @if (auth()->user()->isAdmin !== 2 && $usuarioCarpeta
                == auth()->user()->id)
